...
candidate_form = """{
    "candidates": [
    {
        "id": "Unique identifier of the candidate. It can be just a number starting from 0 and incremented by 1 for each candidate",
        "surface_form": "mention name exactly as it appears in the text", 
        "context": "context of the mention exactly as it appears in the text",
        "version": {"surface_form": "exact form of the version in the text", "context": "context of the version in the text"} or null,
        "publisher": [{"surface_form": "exact form of the publisher in the text", "context": "context of the publisher in the text"}],
        "url": [{"surface_form": "exact form of the URL in the text", "context": "context of the URL in the text"}],
        "language": [{"surface_form": "exact form of the language in the text", "context": "context of the language in the text"}]
    }
    ]
}"""
...
class SearchAgent(Agent):
    ...
    verify_second_pass: bool = ConfigurableValue(
        user_default=True,
        desc="Whether to verify candidates in the second pass."
    )
...
Extract also additional information about the software mentions if available in the text. The additional information are:
...
Please extract all software mentions from the text.""")
                ...
                LiteralScalarString("""[
    {
        ...
        "url": [
            {
                "surface_form": "https://ollama.com/docs/api",
                "context": "API [https://ollama.com/docs/api] to run",
            }
        ],
        "language": []
    }
        ...
        "url": [
            {
                ...
                "context": "available at https://example.com, the whole",
            }
        ],
        "language": [
            {
...
{% endif %}"""),
    ...
    verification_system_prompt: Template = ConfigurableValue(
        user_default=LiteralScalarString(f"""You are a software mention verification agent. Your task is to verify if the given mention is a software or not and whether all extracted components (version, publisher, URL, language) are correct.
...
It is possible to fix components of the mention if they are incorrect, but it must be based on the information in the text.
"""),
...
Please decide whether this candidate is a software mention or not and fix its components if necessary:
{
    "id": 1,
    "surface_form": "Transformers",
    "context": "open-source Transformers library",
...
""")
    ...
    "reason": "The candidate 'Transformers' is verified as a software mention because it is a named software component/package. The context 'open-source Transformers library' indicates that it is a software library. The publisher 'Hugging Face' is a well-known organization in the field of machine learning and natural language processing, which further supports the identification of 'Transformers' as a software mention. The language 'Python' is not associated to Transformers library, but to the MLTrainer mention, so it is not relevant for this candidate.",
    "is_software": true,
    "verified": {
        "id": 1,
        "surface_form": "Transformers",
...
}
{
    ...
    "version": {
        "surface_form": "2.0",
        "context": "version 2.0 developed"
    },
    "publisher": [
        {
            "surface_form": "FastSearch Inc.",
            "context": "by our team FastSearch Inc. is available"
        }
    ],
    "url": [
        {
            "surface_form": "https://example.com/bintreesearch",
            "context": "available at https://example.com/bintreesearch."
        }
    ],
    "language": []
}
...
All software mentions candidates are:
{
    ...
    "version": {
        "surface_form": "1.2.3",
        "context": "version 1.2.3 developed"
    },
    "publisher": [
        {
            "surface_form": "SecureSoft",
            "context": "by SecureSoft is available"
        }
    ],
    "url": [
        {
...
"""),
    ...
    "reason": "The candidate 'CryptoSeek' is verified as a software mention. The search results indicate that 'CryptoSeek' is an open-source software solution for secure communication. The mention context seems to be slightly off, but it is clear that it refers to the second occurrence of 'CryptoSeek' in the text. The version '1.2.3', publisher 'SecureSoft', URL 'https://securesoft.com/cryptoseek', and language 'Rust' are missing in the target candidate and should be added based on the information provided in the text.",
    "is_software": true,
    "verified": {
        "id": 2,
        "surface_form": "CryptoSeek",
...
Please verify the following mention and fix its components if necessary:
{
    "id": "second_pass_2",
    "surface_form": "Hadoop",
    ...
    "reason": "The candidate 'Hadoop' is verified as a software mention. The search results indicate that 'Hadoop' is an open-source software and mention with the same surface form was already extracted as software mention in the first pass. There is missing version component in the target candidate",
    "is_software": true,
    "verified": {
        "id": "second_pass_2",
        "surface_form": "Hadoop",
...
Please verify the following mention and fix its components if necessary:
{
    ...
    "reason": "The candidate 'tidyverse' is verified as a software mention. The search results indicate that 'tidyverse' is a collection of R packages designed for data science. The version '1.3.0', publisher 'RStudio', and URL 'https://www.tidyverse.org/' are missing in the target candidate and should be added based on the information provided in the text.",
    "is_software": true,
    "verified": {
        "id": "second_pass_3",
        "surface_form": "tidyverse",
        ...
        "url": [
            {
                "surface_form": "https://www.tidyverse.org/",
                "context": "available at https://www.tidyverse.org/."
            }
        ],
        "language": [
            {
...
Please create a search query for the mention."""),
    ...
    def __call__(self, text: str) -> list[Mention]:
        ...
        verified_candidates = self.verify_candidates(text, candidates) if self.verify else candidates
    ...
    def call_second_pass(self, text: str, all_document_mentions: list[Mention], text_start_offset: int) -> list[
        ...
        verified = self.verify_second_pass_mentions(text, searched_candidates, input_text_mentions,
                                                    all_document_mentions) if self.verify_second_pass else searched_candidates
    ...
    def convert_candidate_to_mention(self, text: str, candidate: Candidate) -> Mention:
        ...
        version, publisher, urls, language = None, [], [], []

        if candidate.version is not None:
            try:
                version = assemble_additional_info(candidate.version.context, candidate.version.surface_form, "Version")
            except ValueError as e:
                print(f"Warning: {e}", file=sys.stderr)
        for p in candidate.publisher:
            try:
                publisher.append(assemble_additional_info(p.context, p.surface_form, "Publisher"))
            except ValueError as e:
                print(f"Warning: {e}", file=sys.stderr)
        for u in candidate.url:
            try:
                urls.append(assemble_additional_info(u.context, u.surface_form, "URL"))
            except ValueError as e:
                print(f"Warning: {e}", file=sys.stderr)
        for l in candidate.language:
            try:
                language.append(assemble_additional_info(l.context, l.surface_form, "Language"))
            except ValueError as e:
                print(f"Warning: {e}", file=sys.stderr)
        ...
        while True and cnt < self.max_repairs:
            ...
            try:
                for f in [
                    ...
                    self.repair_ambiguous_contexts, self.repair_overlapping_surface_forms, self.repair_urls
                ]:
    ...
    def repair_urls(self, text: str, candidates: list[Candidate]) -> bool:
        """
